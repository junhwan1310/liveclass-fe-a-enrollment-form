...
테스트 범위는 과제의 핵심 로직을 우선 검증하는 방향으로 설정했습니다.

유효성 검증, Mock API 예외 처리, 제출 에러 메시지 변환은 Vitest 기반 단위 테스트로 확인했고, 실제 사용자 흐름은 수동 테스트 체크리스트로 검증했습니다.
...
- 자동화된 브라우저 E2E 테스트는 이번 제출 범위에 포함하지 않았습니다.
  - 대신 주요 유효성 검증, Mock API 예외 처리, 제출 에러 메시지 변환은 Vitest 단위 테스트로 검증했습니다.
  - 개인 신청, 단체 신청, 정원 초과, 중복 신청, 임시 저장 복구 등 실제 사용자 흐름은 수동 테스트 체크리스트로 확인했습니다.
...
본 과제에서는 AI 도구를 전반적인 개발 보조 도구로 활용했습니다.

활용 범위는 다음과 같습니다.

- 과제 요구사항 분석 및 체크리스트 정리
- 프로젝트 폴더 구조 설계 보조
- TypeScript 타입 정의 초안 작성 보조
- Mock 데이터 및 Mock API 흐름 설계 보조
- React 컴포넌트 구현 초안 작성 보조
- 개인 신청 / 단체 신청 조건부 필드 처리 방식 검토
- Zod 기반 유효성 검증 schema 설계 보조
- 정원 초과, 중복 신청, 참가자 이메일 중복 등 예외 상황 점검
- localStorage 기반 임시 저장 및 이탈 방지 흐름 검토
- React Fast Refresh lint 오류 원인 파악 및 수정 방향 검토
- 테스트 케이스 초안 작성 보조
- README 초안 작성 및 문구 정리 보조
- 면접 대비를 위한 코드 구조 설명 정리

단, AI가 제안한 내용을 그대로 제출하지 않고, 과제 요구사항과 비교하면서 직접 실행 및 검증했습니다.

특히 아래 항목은 로컬 환경에서 직접 확인했습니다.

- 개인 신청 정상 제출
- 단체 신청 정상 제출
- 개인 신청에서 단체 필드가 검증되지 않는지 확인
- 단체 신청 참가자 이메일 중복 검증 확인
- 남은 좌석보다 단체 신청 인원이 많을 때 제출 차단 확인
- `duplicate@test.com` 입력 시 중복 신청 에러 확인
- 제출 실패 시 입력 데이터 유지 확인
- 새로고침 후 입력값 복구 확인
- `npm run test` 성공 여부 확인
- `npm run lint` 성공 여부 확인
